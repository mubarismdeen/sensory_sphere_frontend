import 'package:flutter/material.dart';

const Color mainTitleColor = Color(0xFFA4A6B3);
const Color light = Colors.white;
const Color dark = Colors.black;
const Color lightGrey = Color(0xFFA4A6B3);
const Color highlightedColor = Color.fromRGBO(107, 48, 255, 1);
const Color active = Color(0xFF3C19C0);
const Color shadowColor = Colors.blueGrey;
const Color themeColor = Color.fromRGBO(44, 45, 46, 1);
const Color cardColor = Color(0xFF37474F);
const Color inputFieldOutlineColor = Color.fromRGBO(107, 48, 255, 1);
const Color errorColor = Colors.red;
const Color locationButtonColor = Colors.green;
const Color green = Colors.lightGreenAccent;
const Color elevatedButtonColor = Color.fromRGBO(107, 48, 255, 0.3);
const Color appBarColor = Color.fromRGBO(59, 38, 115, 1);

const TextStyle tableHeaderStyle = TextStyle(
    fontStyle: FontStyle.italic, fontWeight: FontWeight.w900, color: lightGrey);
const TextStyle rowTextStyle = TextStyle(color: Colors.black);
